...
import { 
  ...
  X, Edit2, Trash2, Plus, ListOrdered, ShieldCheck
} from 'lucide-react';
...
const ManajemenKantor = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    // Pastikan nama tabel konsisten: 'kantor' atau 'master_shift'
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-4 animate-in fade-in duration-500 text-left">
                ...
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-400">ID: #SHIFT-{s.id}</p>
                        </div>
                    </div>
                    <div className="flex space-x-2">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl my-auto animate-in zoom-in duration-300 overflow-hidden">
            <form onSubmit={handleSubmit}>
                    ...
                    {activeTab === 'KANTOR' ? (
                        <>
                            <div className="space-y-2">
                              <label className="text-[10px] font-black uppercase text-blue-600 ml-2">Detail Identitas</label>
                        ...
                        <>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-blue-600 ml-2">Nama Shift</label>
                                <input required placeholder="Contoh: Shift Pagi / Middle" className="w-full bg-slate-50 border-2 border-slate-100 focus:border-blue-600 rounded-2xl px-6 py-4 font-bold outline-none transition-all" value={formData.nama_shift} onChange={e => setFormData({...formData, nama_shift: e.target.value})} />
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 focus-within:border-emerald-500 transition-all">
                                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-2">Pukul Masuk</label>
                                ...
                                <div className="p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 focus-within:border-rose-500 transition-all">
                                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-2">Pukul Pulang</label>
                ...
                <div className="p-10 bg-slate-50 flex justify-end gap-4">
                    <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-4 font-bold text-slate-400 uppercase text-xs">Batal</button>
                    <button type="submit" disabled={loading} className="bg-slate-900 hover:bg-blue-600 text-white px-12 py-4 rounded-2xl font-black uppercase text-xs tracking-[0.2em] transition-all flex items-center shadow-lg active:scale-95">
